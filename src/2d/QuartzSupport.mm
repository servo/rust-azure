...
bool MacIOSurfaceLib::isInit() {
  ...
  if (!sIOSurfaceFramework) {
    NSLog(@"MacIOSurfaceLib failed to initialize");
  }
...
bool nsCARenderer::SetupRenderer(void *aCALayer, int aWidth, int aHeight,
                                     double aContentsScaleFactor,
...
bool nsCARenderer::Render(int aWidth, int aHeight, 
                          double aContentsScaleFactor,
                          CGImageRef *aOutCGImage) {
  if (!aOutCGImage && !mIOSurface) {
    NSLog(@"No target destination for rendering");
  } else if (aOutCGImage) {
...
bool nsCARenderer::DrawSurfaceToCGContext(CGContextRef aContext, 
                                          MacIOSurface *surf, 
