...
extern "C" {
    ...
    pub fn AzCreateDrawTargetSkiaWithGrContextAndFBO(aGrContext: SkiaGrContextRef,
                                                     aFBOID: u32,
                                                     aSize: *mut AzIntSize,
                                                     aFormat: AzSurfaceFormat)
                                                     -> AzDrawTargetRef;
    ...
    pub fn AzDrawTargetStrokeRect(aDrawTarget: AzDrawTargetRef,
                                  aRect: *mut AzRect,
                                  aPattern: AzPatternRef,
                                  aStrokeOptions: *mut AzStrokeOptions,
                                  aDrawOptions: *mut AzDrawOptions);

    pub fn AzDrawTargetStrokeLine(aDrawTarget: AzDrawTargetRef,
                                  aStart: *mut AzPoint,
                                  aEnd: *mut AzPoint,
                                  aPattern: AzPatternRef,
                                  aStrokeOptions: *mut AzStrokeOptions,
    ...
    pub fn AzDrawTargetFill(aDrawTarget: AzDrawTargetRef,
                            aPath: AzPathRef,
                            aPattern: AzPatternRef,
                            aOptions: *mut AzDrawOptions);

    pub fn AzDrawTargetStroke(aDrawTarget: AzDrawTargetRef,
                              aPath: AzPathRef,
                              aPattern: AzPatternRef,
                              aStrokeOptions: *const AzStrokeOptions,
                              aDrawOptions: *const AzDrawOptions);
    ...
    pub fn AzDrawTargetFillGlyphs(aDrawTarget: AzDrawTargetRef,
                                  aFont: AzScaledFontRef,
                                  aGlyphBuffer: *mut AzGlyphBuffer,
                                  aPattern: AzPatternRef,
                                  aOptions: *mut AzDrawOptions,
                                  aRenderingOptions: AzGlyphRenderingOptionsRef);

    pub fn AzDrawTargetDrawSurface(aDrawTarget: AzDrawTargetRef,
                                   aSurface: AzSourceSurfaceRef,
                                   aDest: *mut AzRect,
                                   aSource: *mut AzRect,
                                   aSurfOptions: AzDrawSurfaceOptionsRef,
                                   aOptions: *mut AzDrawOptions);

    pub fn AzDrawTargetCopySurface(aDrawTarget: AzDrawTargetRef,
                                   aSurface: AzSourceSurfaceRef,
                                   aSource: *mut AzIntRect,
                                   aDestination: *mut AzIntPoint);
    ...
    pub fn AzDrawTargetCreateSourceSurfaceFromData(aDrawTarget: AzDrawTargetRef,
                                                   aData: *const u8,
                                                   aSize: *mut AzIntSize,
                                                   aStride: i32,
                                                   aFormat: AzSurfaceFormat)
    ...
    pub fn AzDrawTargetCreateFilter(aDrawTarget: AzDrawTargetRef,
                                    aFilterType: AzFilterType)
                                    -> AzFilterNodeRef;
    ...
    pub fn AzCloneSurfacePattern(aPattern: AzSurfacePatternRef) -> AzSurfacePatternRef;
    ...
    pub fn AzCreateScaledFontForNativeFont(aNativeFont: *mut AzNativeFont,
                                           aSize: AzFloat)
                                           -> AzScaledFontRef;

    pub fn AzCreateScaledFontForTrueTypeData(aFontData: *const u8,
                                             aFontDataSize: u32,
                                             aFaceIndex: u32,
                                             aGlyphSize: f32,
                                             aType: enum_AzFontType)
                                             -> AzScaledFontRef;
    ...
    pub fn AzCreateFontOptionsForName(aName: *const c_char,
                                      aStyle: enum_AzFontStyle)
                                      -> *mut AzFontOptions;
    ...
    pub fn AzPathContainsPoint(aPath: AzPathRef,
                               aPoint: *mut AzPoint,
                               aMatrix: *const AzMatrix)
                               -> bool;
